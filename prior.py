...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 21:36:00 2019
...
def gaussian_mixture2(batch_size, n_dim=2, n_labels=5, x_var=0.5, y_var=0.1, label_indices=None):
    if n_dim != 2:
        raise Exception("n_dim must be 2.")
    ...
    def sample(x, y, label, n_labels):
        ...
        #shift = 3.0
        r = 2.0 * np.pi / float(n_labels) * float(label)
        #r = 5.0 * np.pi / float(n_labels) * float(label)
        new_x = x * cos(r) - y * sin(r)
        new_y = x * sin(r) + y * cos(r)
        new_x += shift * cos(r)
        new_y += shift * sin(r)
